...
def main(config, fold, artifact_path, feature_aggregation, timepoints):   
    ...
    train_dl = torch.utils.data.DataLoader(train_dataset, batch_size=4, shuffle=True, num_workers=4)
    ...
    val_dl = torch.utils.data.DataLoader(val_dataset, batch_size=4, shuffle=False, num_workers=4)
    ...
    test_dl = torch.utils.data.DataLoader(test_dataset, batch_size=4, shuffle=False, num_workers=4)
    
    # model set-up
    # model = build_model(mtan_masking=config.mtan_masking, filters=config.filters, device=config.device)

    if config.kaczmarek:
        model = ResNet18EncoderKaczmarek(timepoints=timepoints, training=False).to(config.device)
    ...
    if config.use_gnn:             
        model_gnn = TemporalGNN(in_channels=960, hidden_channels=512, out_channels=128, num_layers=3).to(config.device)
        pretrained_model_path_gnn = artifact_path.replace('model', 'model_gnn')
        model_gnn.load_state_dict(torch.load(pretrained_model_path_gnn, map_location=config.device))
        model_gnn.eval()   
        ...
        for batch_data in tqdm(train_dl):        
            ...
            # if not config.use_gnn:       
                ...
                if config.kaczmarek:
                    ...
                    logits, latent_original, latent_transformed, _ = model(images)
                    latents = latent_original  
                    ...
                    graph_data = make_directed_complete_forward_graph(latents, batch_size=batch_size)
                    graph_data = graph_data.to(device)
                    graph_data = graph_data.sort(sort_by_row=False)
                    out = model_gnn(graph_data.x, graph_data.edge_index)
                    latents = out.view(batch_size, -1, out.size(-1))
                    # latents = out.flatten(start_dim=1)      
...
if __name__ == '__main__': 
    ...
    # artifact_dict = {
    #     "fold_0_latest": "/media/johannes/SSD_2.0TB/GNN_pCR/mlruns/704497078155278981/396efa01a66248edbc82ba9342429790/artifacts/model_best.pt",        
    #     "fold_0_best": "/media/johannes/SSD_2.0TB/GNN_pCR/mlruns/704497078155278981/396efa01a66248edbc82ba9342429790/artifacts/model_best.pt",        
        
    #     "fold_1_latest": "/media/johannes/SSD_2.0TB/GNN_pCR/mlruns/704497078155278981/baf342467d1847f783df9b1a95bd63b3/artifacts/model_best.pt",        
    #     "fold_1_best": "/media/johannes/SSD_2.0TB/GNN_pCR/mlruns/704497078155278981/baf342467d1847f783df9b1a95bd63b3/artifacts/model_best.pt",        
        
    #     "fold_2_latest": "/media/johannes/SSD_2.0TB/GNN_pCR/mlruns/704497078155278981/d85369a1b31844468dcbf81bd9a07fd9/artifacts/model_best.pt",        
    #     "fold_2_best": "/media/johannes/SSD_2.0TB/GNN_pCR/mlruns/704497078155278981/d85369a1b31844468dcbf81bd9a07fd9/artifacts/model_best.pt",        
        
    #     "fold_3_latest": "/media/johannes/SSD_2.0TB/GNN_pCR/mlruns/704497078155278981/524b2b065e3f495f8770d43e9336c992/artifacts/model_best.pt",        
    #     "fold_3_best": "/media/johannes/SSD_2.0TB/GNN_pCR/mlruns/704497078155278981/524b2b065e3f495f8770d43e9336c992/artifacts/model_best.pt",        
        
    #     "fold_4_latest": "/media/johannes/SSD_2.0TB/GNN_pCR/mlruns/704497078155278981/6caad90705ea40f49b3346d4186d9e07/artifacts/model_best.pt",        
    #     "fold_4_best": "/media/johannes/SSD_2.0TB/GNN_pCR/mlruns/704497078155278981/6caad90705ea40f49b3346d4186d9e07/artifacts/model_best.pt",        
    # }
    
    artifact_dict = {
               "fold_0_best_metric": "/media/johannes/SSD_2.0TB/GNN_pCR/mlruns/231488041483276162/de043c30804e46a0aa7684d9a6d64929/artifacts/model_best_metric.pt",
               "fold_1_best_metric": "/media/johannes/SSD_2.0TB/GNN_pCR/mlruns/231488041483276162/5479236f073544efae2486eeae64b020/artifacts/model_best_metric.pt",
               "fold_2_best_metric": "/media/johannes/SSD_2.0TB/GNN_pCR/mlruns/231488041483276162/7fe714bd5311461e88e647105bf8aee3/artifacts/model_best_metric.pt",
               "fold_3_best_metric": "/media/johannes/SSD_2.0TB/GNN_pCR/mlruns/231488041483276162/3e1a89250d19431bb61ad289d15e80a1/artifacts/model_best_metric.pt",
               "fold_4_best_metric": "/media/johannes/SSD_2.0TB/GNN_pCR/mlruns/231488041483276162/2c2b878715f94f7e92f069f22ca35e14/artifacts/model_best_metric.pt",
    }
    ...
    assert not (config.use_gnn and config.kaczmarek), "Choose either GNN or Kaczmarek model!" 

    for artifact_name, artifact_path in artifact_dict.items():
        # for feature_aggregation in ['concat', 'mean']:
        for feature_aggregation in ['mean', 'concat']:
            for timepoints in [4]:
                ...
                if config.kaczmarek:
                    mlflow.set_experiment("evaluation_Kaczmarek")
                elif config.use_gnn:
                    mlflow.set_experiment("evaluation_GNN")
                else:
                    mlflow.set_experiment("evaluation_Ivana")
                mlflow.start_run()
                mlflow.log_param("fold", fold)
                mlflow.log_param("model_checkpoint", model_checkpoint)
            
                main(config, fold, artifact_path, feature_aggregation, timepoints)
                mlflow.end_run()
