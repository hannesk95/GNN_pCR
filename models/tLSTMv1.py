...
if __name__ == "__main__":
    ...
    inputs = torch.randn(B, T, C, H, W).cuda()
    labels = torch.randn(B, T, 2).cuda()
    time = torch.randn(B, T, 1).cuda()
    
    model = tLSTMv1(input_size=256, hidden_size=256, num_layers=1, output_size=5, batch_first=True, num_labels=2).cuda()   
    model.init_hidden_()

    output, hidden = model(inputs, labels, time)
    print(output.size())
